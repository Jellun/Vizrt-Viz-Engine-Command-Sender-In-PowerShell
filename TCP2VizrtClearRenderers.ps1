# This script clears Viz Engine Renderer cache for better rendering performance
# Supported Viz commands can be found at https://documentation.vizrt.com/viz-engine-guide-3.14.pdf
# To run this script in Windows, %SystemRoot%\system32\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -NoProfile -file TCP2VizrtClearRenderers.ps1
...
try
{
    ...
    $Commands = "0 RENDERER*FRONT_LAYER SET_OBJECT`0"
    ...
    $Commands = "0 RENDERER*MAIN_LAYER SET_OBJECT`0"
    $BufferSend = $Enc.GetBytes($Commands)
    "Sending command: {$Commands} to Viz Engine"
    $SentBytes = $script:sock.Send($BufferSend)
    "{0} characters sent to: {1} " -f $SentBytes,$engine
    ...
    $Commands = "0 RENDERER*BACK_LAYER SET_OBJECT`0"
    $BufferSend = $Enc.GetBytes($Commands)
    "Sending command: {$Commands} to Viz Engine"
    $SentBytes = $script:sock.Send($BufferSend)
    "{0} characters sent to: {1} " -f $SentBytes,$engine
    ...
    $Commands = "0 MAPS CACHE CLEANUP`0"
    $BufferSend = $Enc.GetBytes($Commands)
    "Sending command: {$Commands} to Viz Engine"
    $SentBytes = $script:sock.Send($BufferSend)
    "{0} characters sent to: {1} " -f $SentBytes,$engine
